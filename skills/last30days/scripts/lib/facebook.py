...
Facebook credentials remain in the operator-managed browser profile. Shared
workspace lifecycle belongs to `agent_browser_runtime`; this module owns only
Facebook authentication, navigation, extraction, and post quality. It never
reads or returns browser cookie values or raw page HTML.
"""
...
MAX_RUN_BUDGET_SECONDS = browser_runtime.MAX_RUN_BUDGET_SECONDS
REMOTE_VIEW_RECONCILIATION_RESERVE_SECONDS = (
    browser_runtime.REMOTE_VIEW_RECONCILIATION_RESERVE_SECONDS
)
TAB_INVENTORY_TIMEOUT_SECONDS = browser_runtime.TAB_INVENTORY_TIMEOUT_SECONDS
QUERY_CAPTURE_OUTER_TIMEOUT_SECONDS = 50
QUERY_CAPTURE_INNER_TIMEOUT_MS = 45_000
SERVICE_EVALUATE_MAX_RETURN_BYTES = browser_runtime.SERVICE_EVALUATE_MAX_RETURN_BYTES
SERVICE_TAB_READY_TIMEOUT_MS = browser_runtime.SERVICE_TAB_READY_TIMEOUT_MS
SERVICE_TAB_READY_OUTER_TIMEOUT_SECONDS = (
    browser_runtime.SERVICE_TAB_READY_OUTER_TIMEOUT_SECONDS
)
ERROR_TYPES = browser_runtime.ERROR_TYPES
RATE_LIMIT_REASONS = browser_runtime.RATE_LIMIT_REASONS
agent_browser_config = browser_runtime.agent_browser_config
...
class BrowserWorkspaceRequest(browser_runtime.BrowserWorkspaceRequest):
    """Provider-neutral request with Facebook-compatible defaults."""
...
FacebookScraperFailure = browser_runtime.AgentBrowserRuntimeFailure


class AgentBrowserClient(browser_runtime.AgentBrowserClient, Protocol):
...
class CliAgentBrowserClient(browser_runtime.CliAgentBrowserClient):
    """Facebook-specific behavior layered on the provider-neutral runtime."""

    def __init__(self, *, timeout: int, job_timeout_ms: int | None = None) -> None:
        super().__init__(timeout=timeout, job_timeout_ms=job_timeout_ms)
        ...
        if matches:
            try:
                ...
                _log(
                    ...
                    "inspecting authentication once on a fresh target"
                )
...
_session_has_ambiguous_browser_ownership = (
    browser_runtime._session_has_ambiguous_browser_ownership
)
_loopback_cdp_port = browser_runtime._loopback_cdp_port
_select_target_id = browser_runtime._select_target_id
_browser_workspace_from_retained_owner = browser_runtime._browser_workspace_from_retained_owner
_is_target_service_url = browser_runtime._is_target_service_url
_exact_retained_default_owner = browser_runtime._exact_retained_default_owner
_url_matches_hostname = browser_runtime._url_matches_hostname
_ready_operator_stream = browser_runtime._ready_operator_stream
_has_ready_operator_stream = browser_runtime._has_ready_operator_stream
_select_live_route_entry = browser_runtime._select_live_route_entry
_profile_scoped_session_name = browser_runtime._profile_scoped_session_name
_operator_url = browser_runtime._operator_url
_redact = browser_runtime._redact
_cli_error_message = browser_runtime._cli_error_message
_elapsed_ms = browser_runtime._elapsed_ms
_is_navigation_timeout = browser_runtime._is_navigation_timeout
_command_operation = browser_runtime._command_operation
