...
class MonitorRecords:
    ...
    def get(self, kind, key, partition):
        ...
        try:
            row = conn.execute("SELECT * FROM service_monitor_records WHERE kind=? AND record_id=? AND partition_id=?",
                               (kind, key, partition)).fetchone()
        finally:
            conn.close()
        if row is None:
        ...
        if _digest(payload) != row["payload_sha256"]:
            raise MonitorKernelError(c.MonitorErrorCode.IMMUTABLE_CONFLICT, "monitor record integrity failure")
        return payload
    ...
    def put(self, kind, key, partition, owner, payload):
        ...
        try:
            ...
            row = conn.execute("SELECT * FROM service_monitor_records WHERE kind=? AND record_id=?", (kind, key)).fetchone()
            if row:
                if row["payload_sha256"] != _digest(payload) or row["partition_id"] != partition or row["owner_id"] != owner:
                    raise MonitorKernelError(c.MonitorErrorCode.IMMUTABLE_CONFLICT, "immutable monitor record conflict")
            else:
                conn.execute("INSERT INTO service_monitor_records VALUES (?, ?, ?, ?, ?, ?)",
...
def prepare_digest(repository, records, run, spec, *, max_bytes=65_536):
    ...
    refs = {(hit["post_id"], hit["revision_id"]): hit for hit in receipt["evidence_refs"]}
    prior_head = None
    if run.prior_baseline_id:
        prior = repository.get_baseline(run.prior_baseline_id)
        prior_head = prior.evidence_head_id
        prior_receipt = repository.capture_receipt(prior.snapshot_id)
        refs.update({(hit["post_id"], hit["revision_id"]): hit for hit in prior_receipt["evidence_refs"]})
    comparison = run.comparison
    result = {"schema_version": 1, "digest_id": "digest-" + _digest([run.run_id, "deterministic_v1"])[:32],
              "run_id": run.run_id, "monitor_id": run.monitor_id,
              "access_partition_id": spec.access_partition_id, "renderer_version": "deterministic_v1",
              "comparator_version": comparison.comparator_version,
              "comparison_status": comparison.status.value,
              "prior_baseline_id": run.prior_baseline_id, "candidate_baseline_id": run.candidate_baseline_id,
              "prior_head_id": prior_head, "current_head_id": snapshot["evidence_head_id"],
              "knowledge_cutoff": snapshot["knowledge_cutoff"],
              "coverage_start": snapshot["coverage_start"], "coverage_end": snapshot["coverage_end"],
              "coverage": snapshot["coverage"], "coverage_scope": "bounded_cache_view_only",
              "prior_count": comparison.prior_count, "current_count": comparison.current_count,
              "counts": {kind.value: len(comparison.by_kind(kind)) for kind in c.ChangeKind},
              "omitted_count": len(comparison.changes), "entries": []}
    for change in comparison.changes:
        links = []
        causes = set()
        for version in dict.fromkeys((change.prior_version_id, change.current_version_id)):
            if version is None:
                continue
            hit = refs.get((change.evidence_id, version))
            if hit is None or hit["access_partition_id"] != spec.access_partition_id:
                raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "digest evidence unavailable")
            links.append(hit["evidence_ref"])
            causes.update(hit["collection_refs"])
        entry = {**change.to_dict(), "evidence_refs": links, "collection_refs": sorted(causes)}
        result["entries"].append(entry)
        result["omitted_count"] -= 1
        if len(result["entries"]) > spec.max_items or len(_json(result).encode()) > max_bytes:
            result["entries"].pop()
            result["omitted_count"] += 1
            break
    if len(_json(result).encode()) > max_bytes:
        raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "digest exceeds byte bound")
    return records.put("digest", run.run_id, spec.access_partition_id, spec.monitor_id, result)
