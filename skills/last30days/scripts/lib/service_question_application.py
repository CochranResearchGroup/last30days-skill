...
def _bounded(result):
    if (
        len(contracts.canonical_json(result.to_dict()).encode("utf-8"))
        > MAX_RESPONSE_BYTES
    ):
...
class _UnavailableWorker:
    ...
    def answer(self, payload):
        raise QuestionWorkerError(
            "model_unavailable", "question synthesis is not configured", retryable=False
        )
...
class QuestionApplication:
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._run, name="last30days-question-worker"
            )
            self._thread.start()
    ...
    def stop(self, *, timeout: float = 5.0) -> bool:
        ...
        if (
            ...
            raise ValueError(
                "question shutdown timeout must be between 0 and 30 seconds"
            )
        with self._lock:
            self._stopping.set()
            self._wake.set()
        ...
        try:
            while not self._stopping.is_set():
                ...
                if status is None:
                    ...
                    self._wake.clear()
        except Exception:  # noqa: BLE001 - fail closed without exposing worker payloads
    ...
    def _partitions(self, profile_id: str) -> tuple[str, ...]:
        ...
        if not partitions or not all(
            isinstance(item, str) and item for item in partitions
        ):
    ...
    def ask_question(
        self,
        request: contracts.QuestionRequestV1,
        *,
        ...
        with self._lock:
            ...
            scoped = replace(
                request,
                request_id=contracts.stable_id(
                    "question-request",
                    {
            ...
            status = self.service.submit(
                scoped, access_partitions=self._partitions(request.profile_id)
            )
            if status.state == "pending":
                if self._worker_configured:
                    ...
                    self.runner.run_once(
                        worker_id="unconfigured-host", question_id=status.question_id
                    )
        deadline = time.monotonic() + request.limits.wait_ms / 1000
        while True:
            status = self.question_status(
                status.question_id, profile_id=request.profile_id
            )
    ...
    def question_status(
        ...
        with closing(
            sqlite3.connect(f"{self.db_path.as_uri()}?mode=ro", uri=True, timeout=5)
        ) as conn:
        ...
        if row is None:
            raise QuestionUnavailableError("question unavailable")
        if json.loads(row[0])["profile_id"] != profile_id or row[1] != contracts.digest(
            sorted(partitions)
        ):
            raise QuestionUnavailableError("question unavailable")
    ...
    def read_evidence(
        self, request: contracts.EvidenceReadRequestV1
    ) -> contracts.EvidenceReadResponseV1:
        return _bounded(
            self.resolver.read(
                request, access_partitions=self._partitions(request.profile_id)
            )
        )
