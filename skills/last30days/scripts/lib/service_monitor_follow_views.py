...
class FollowDefinition:
    ...
    def to_dict(self):
        return {"schema_version": 1, "view_ref": self.view_ref.to_dict(),
                "collection_spec": self.spec.to_dict(), "spec_digest": self.spec.spec_digest,
                "search": {"profile_id": self.spec.profile_id, "query": None,
                           "filters": {"sources": [self.spec.source],
                                       "collection_refs": ["legacy:spec:" + self.spec.collection_spec_id]},
                           "page_size": 100, "sort": "observed_desc", "revision_mode": "current"}}
...
class FollowViewProvider(SavedQueryViewProvider):
    ...
    def definition(self, ref, partition, *, readable=True):
        if self.collection_reader is None:
            raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "follow view unavailable")
        try:
            spec = self.collection_reader.get_spec_revision(ref.collection_spec_id, ref.spec_version)
            current = self.collection_reader.get_spec(ref.collection_spec_id)
        except KeyError:
            raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "follow view unavailable") from None
        if spec.access_partition_id != partition or current.access_partition_id != partition:
            raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "follow view unavailable")
        if spec.collection_purpose != "tailored_follow" or spec.is_quarantined_follow:
            raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "follow view unavailable")
        if readable and (current.lifecycle_state != "active" or current.is_quarantined_follow
    ...
    def capture(self, ref, access_partition_id, capture_id):
        ...
        snapshot_id = "follow-view-" + _digest([ref.to_dict(), access_partition_id, capture_id])[:32]
        ...
        try:
            ...
            self.repository.finish_capture(snapshot_id, error_code="follow_capture_failed")
            if isinstance(exc, MonitorKernelError):
                raise
            raise MonitorKernelError(c.MonitorErrorCode.VIEW_UNAVAILABLE, "follow capture failed") from exc
